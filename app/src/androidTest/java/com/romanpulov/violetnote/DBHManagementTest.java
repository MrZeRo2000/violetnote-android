...
public final class DBHManagementTest extends DBBaseTest {
    ...
    private void testHNoteItems() {
        ...
        mBasicNoteItemDAO.updateNameValue(noteItem1);
        ...
        //delete the note
        result = mBasicNoteDAO.delete(note);
        assertNotEquals(0, result);

        BasicNoteGroupA group = mBasicNoteGroupDAO.getById(2);

        List<BasicNoteA> notes =  mBasicNoteDAO.getTotalsByGroup(group);
    ...
    private void testHNoteCOItems() {
        ...
        result = mBasicNoteItemDAO.insertWithNote(note, noteItem3);
        assertNotEquals(-1, result);
        ...
        //check items
        result = mBasicNoteItemDAO.updateChecked(noteItem1, true);
        assertNotEquals(-1, result);
        result = mBasicNoteItemDAO.updateChecked(noteItem2, true);
        assertNotEquals(-1, result);

        //checkout items
        mBasicNoteItemDAO.fillNoteDataItemsWithSummary(note);
        mBasicNoteDAO.checkOut(note, note.getItems(), note.getValues());
